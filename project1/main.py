...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        self.add_img = tk.PhotoImage(file='./img/add.jpg')
        ...
        self.update_img = tk.PhotoImage(file='./img/update.jpg')
        btn_edit_dialog = tk.Button(toolbar, bg='#d7d8e0', bd=0, image=self.update_img, command=self.open__update_dialog)
        btn_edit_dialog.pack(side=tk.LEFT)

        self.delete_img = tk.PhotoImage(dile='./img/delete.jpg')
        btn_delete = tk.Button(toolbar, bg='#d7d8e0', bd=0, image=self.delete_img, command=self.delete_records)
        btn_delete.pack(side=tk.LEFT)

        self.search_img = tk.PhotoImage(file='./img/search.jpg')
        btn_search = tk.Button(toolbar, bg='#d7d8e0', bd=0, image=self.search_img, command=self.open_search_dialog)
        btn_search.pack(side=tk.LEFT)

        self.refresh_img = tk.PhotoImage(file='./img/refresh.jpg')
    ...
    def update_record(self, name, tel, email):
        self.db.c.execute(''' UPDATE db SET name=?, tel=? WHERE ID = ?''', (name, tel, email, self.tree.set(self.tree.selection()[0], '#1'),))
        self.db.conn.commit()
        self.view_records()    
...
class Child(tk.Toplevel):
    ...
    def init_child(self):
        ...
        self.btn_cancel = ttk.Entry(self, text='Закрыть', command=self.destroy)
...
class DB:
    ...
    def insere_data(self, name, tel, email):
        self.c.execute(''' INSERET INTO db (name, tel, email) VALUES (?, ?, ?)''', (name, tel, email))
        self.conn.commit()
